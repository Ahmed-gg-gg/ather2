...
-- Safe to rerun where IF NOT EXISTS is available.
...
-- Admin/teacher management policies. Student membership is readable only for the signed-in user.
create policy "staff can read groups" on public.account_groups for select using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.role in ('admin','teacher'))
);
create policy "staff can manage groups" on public.account_groups for all using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.role in ('admin','teacher'))
) with check (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.role in ('admin','teacher'))
);
create policy "staff can read memberships" on public.account_group_members for select using (
  user_id = auth.uid() or exists (select 1 from public.profiles p where p.id = auth.uid() and p.role in ('admin','teacher'))
);
create policy "staff can manage memberships" on public.account_group_members for all using (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.role in ('admin','teacher'))
) with check (
  exists (select 1 from public.profiles p where p.id = auth.uid() and p.role in ('admin','teacher'))
);
