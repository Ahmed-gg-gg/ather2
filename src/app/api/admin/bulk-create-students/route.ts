...
export async function POST(request: Request) {
  ...
  if (profile?.role !== "admin") {
    return NextResponse.json({ error: "الأدمن فقط يقدر ينشئ دفعة حسابات" }, { status: 403 });
  }
  ...
  if (!grade || !groupName || !prefix || !emailDomain || !/^[a-z0-9.-]+$/.test(emailDomain)) {
    return NextResponse.json({ error: "تأكد من الصف والفصل وبادئة الاسم ونطاق البريد" }, { status: 400 });
  }
  if (passwordMode === "same" && sharedPassword.length < 8) {
    return NextResponse.json({ error: "كلمة المرور الموحدة يجب أن تكون 8 أحرف على الأقل" }, { status: 400 });
  }
  ...
  for (let start = 0; start < jobs.length; start += chunkSize) {
    ...
    const results = await Promise.all(chunk.map(async (i) => {
      ...
      const { data, error } = await admin.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        user_metadata: { full_name: name, role: "student", grade },
      });
      return { i, name, email, password, data, error };
    }));

    for (const result of results) {
      const { data, error, email, name, password } = result;
      if (error || !data.user) {
        errors.push(`${email}: ${error?.message ?? "تعذر إنشاء الحساب"}`);
        continue;
      }
  ...
  if (createdIds.length) {
    ...
    if (groupId) {
      const { error } = await admin
        ...
        .upsert(
          createdIds.map((user_id) => ({ group_id: groupId, user_id })),
