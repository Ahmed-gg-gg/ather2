  ...
  return <div className={`min-h-screen ${kidsMode?'kids-bg':'bg-paper'}`} dir={lang==='ar'?'rtl':'ltr'}>
    <div className={kidsMode?'border-b-4 border-[#ffd166] bg-white':'border-b border-line bg-surface'}>
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">{profile?.role==='admin'&&<a href="/dashboard/admin" className="text-sm font-medium text-white bg-green rounded-md px-3.5 py-1.5 shadow-sm">🛠️ لوحة الأدمن</a>}<a href="/dashboard/courses" className="text-sm text-ink-soft border border-line rounded-md px-3.5 py-1.5">{t('courses')}</a><a href="/dashboard/results" className="text-sm text-ink-soft border border-line rounded-md px-3.5 py-1.5">النتائج</a><a href="/dashboard/ai" className="text-sm text-green-text border border-line rounded-md px-3.5 py-1.5">🤖 المساعد</a><a href="/dashboard/notifications" className="text-sm text-ink-soft border border-line rounded-md px-3.5 py-1.5">🔔{unreadCount?` ${unreadCount}`:''}</a>{profile?.role==='parent'&&<a href="/dashboard/parent" className="text-sm text-green-text border border-line rounded-md px-3.5 py-1.5">{t('myChildren')}</a>}<LanguageToggle/><LogoutButton label={t('logout')}/></div>
      </div>
    </div>
