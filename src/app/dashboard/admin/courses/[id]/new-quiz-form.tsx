...
export default function NewQuizForm({ lessonId }: { lessonId: string }) {
  ...
  function setQuestionType(qi: number, type: QuestionType) {
    setQuestions((qs) =>
      qs.map((q, idx) => {
        if (idx !== qi) return q;
        if (type === "true_false") {
          return { ...q, type, options: ["صح", "غلط"], correctIndex: 0 };
        }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { data: quiz, error: quizError } = await supabase
      .from("quizzes")
      .insert({ lesson_id: lessonId, title: quizTitle })
      .select()
      .single();
  ...
  return (
    <form
      ...
      className="border-t border-line mt-4 pt-4 flex flex-col gap-4"
    >
      <input
          ...
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-1.5 text-xs text-ink-soft">
              <input
                ...
                onChange={() => setQuestionType(qi, "multiple_choice")}
              />
              اختيار من متعدد
            </label>
            <label className="flex items-center gap-1.5 text-xs text-ink-soft">
              <input
                ...
                onChange={() => setQuestionType(qi, "true_false")}
              />
              صح / غلط
            </label>
          </div>
          ...
          {q.type === "true_false"
            ? q.options.map((opt, oi) => (
                <div key={oi} className="flex items-center gap-2">
                  ...
                  />
                  <span className="flex-1 px-3 py-2 border border-line rounded-md bg-surface text-sm text-ink-soft">
                    {opt}
                  </span>
                </div>
              ))
      ...
      <button
        type="button"
        className="text-sm text-ink-soft border border-line rounded-md px-3.5 py-1.5 self-start"
      ...
      <div className="flex gap-2">
        <button
          type="submit"
          disabled={loading}
          className="bg-green text-white text-sm font-medium py-2.5 px-5 rounded-md disabled:opacity-60"
        >
          {loading ? "جاري الحفظ…" : "حفظ الكويز"}
        </button>
        <button
          type="button"
